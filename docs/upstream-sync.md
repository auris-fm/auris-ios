...
Upstream Pocket Casts ships minor releases on a **~14-day cadence** (iOS minor releases average ~11 days; Android median 14 days over the 8.x series). Biweekly checks catch each minor release without spamming no-op runs. The preflight step also skips when the latest tag is already merged or an open sync PR exists.
...
```bash
export UPSTREAM_REPO=Automattic/pocket-casts-ios
